...
#navnath
class Current_Stats_Bank_Nifty:
    def __init__(self,root):
        self.root=root
        self.root.title("Bank Nifty")
        ...
        #=================Title======================
        lbl_title=Label(self.root,text="Bank Nifty Historical Data", font=("times new roman",16,"bold"),bg="white",fg="black",bd=4,relief=RIDGE)
        ...
        #====================right side img=============================
        img1=Image.open(r"C:\Users\navna\Music\Nava Services\wallstreet.jpg")
        img1=img1.resize((1000,360),Image.LANCZOS)
        self.photoimg1=ImageTk.PhotoImage(img1)

        lblimg=Label(self.root,image=self.photoimg1,bd=0,relief=RIDGE)
        lblimg.place(x=390,y=330,width=1000,height=360)
        ...
        lblimg.place(x=190,y=420,width=190,height=190)
        
        #=================holdings table frame============================

        Table_frame=LabelFrame(self.root,bd=2,relief=RIDGE,text="Bank Nifty Historical Data And Search System",font=("times new roman",12,"bold"),padx=2)
...
######
    def insert_records(self): #self.insert_records 
        if self.var_date.get()=="" or self.var_day_name.get()=="": 
            ...
            try: 
                ...
                my_cursor.execute("insert into Bank_Nifty(date, day, open, high, low, close, day_change, change_prct) values(%s,%s,%s,%s,%s,%s,%s,%s)",( 
                    self.var_date.get(), 
    ...
    def fetch_data(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="MySQL$2022",database="Indian_Indices")
        my_cursor=conn.cursor()
        my_cursor.execute("select Bank_Nifty_id,Date,Day,Open,High,Low,Close,Day_Change,Change_Prct from Bank_Nifty")
    ...
    def update(self):
        if self.var_serial_id.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="MySQL$2022",database="Indian_Indices")
            my_cursor=conn.cursor()
            my_cursor.execute("update Bank_Nifty set Bank_Nifty_id=%s,Day=%s where Date=%s",(
    ...
    def search(self):
        ...
        my_cursor.execute("select * from Bank_Nifty where "+str(self.search_var.get())+" LIKE '%"+str(self.txt_search.get())+"%'")
        rows=my_cursor.fetchall()
        if len (rows)!=0:
            self.Record_Details_Table.delete(*self.Record_Details_Table.get_children())
            for i in rows:
...
if __name__ == "__main__":
    root=Tk()
    obj=Current_Stats_Bank_Nifty(root)
    root.mainloop()